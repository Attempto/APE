...
This program compares a stored and manually checked DRS (taken from the APE regression test set)
with an automatically generated DRS for the same ACE text.
If they do not match then a failure message along with both DRSs are displayed.
The matching algorithm is defined in the APE module =|utils/are_equivalent.pl|=.

Configuration:

* change the first line of this program to match your SWI-Prolog installation
* change user:file_search_path/2 (first two rules in this program)
...
Read the PlDoc comments in the source code to find out how to interpret the result codes
...
%
% Applies APE to every ACE text that is a part of a positive regression test case,
% and compares the resulting DRS to the DRS that is stored in the regression test set.
%
main :-
	time_limit(TimeLimit),
	get_time(Timestamp),
	format_time(atom(FormattedTimestamp), 'Timestamp: %F %T%z', Timestamp),
	writeln(FormattedTimestamp),
	forall(
...
run_test(TimeLimit, Number, Text, Drs) :-
		...
		call_with_time_limit(
			TimeLimit,
			(
				acetext_to_drs(Text, _, _, RealDrs, Messages),
	...
	display_result(Result, Number, Text, Messages),
	(
		Result = '####'
	->
		display_drs(Drs), nl,
...
%% display_results(+ResultCode:atom, +Number:number, +Text:atom, +Messages:list) is det.
%
% Pretty-prints the result of DRS comparison.
% Note: Messages are currently not output.
%
% @param ResultCode is one of {=|0000|=, =|0->#|=, =|#->0|=, =|####|=, =|----|=}
% @param Number is the ID-number of the test case
% @param Text is the ACE text
% @param Messages is a list of possible parser error messages
%
display_result(Result, Number, Text, _) :-
	format('~n~*t~d~6| [~w] ~q~n', [0'0, Number, Result, Text]).

%display_result(Result, Number, Text, [Message | Messages]) :-
%	format('~n~*t~d~6| [~w] ~q ~w~n', [0'0, Number, Result, Text, [Message | Messages]]).
